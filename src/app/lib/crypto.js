...
export async function encryptNote(text, password) {
    ...
    const derivedKey = await window.crypto.subtle.deriveKey(
      {
        name: 'PBKDF2',
        salt: encoder.encode('salt'), // you can replace with a random salt stored alongside ciphertext for better security
        iterations: 100000,
        hash: 'SHA-256',
      },
      keyMaterial,
  ...
  export async function decryptNote(encrypted, iv, password) {
    ...
    // Convert base64 encrypted string back to Uint8Array
